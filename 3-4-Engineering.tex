...
% Note that the size of $K$ is a parameter that we can tune: a smaller $K$ means a smaller seeding process, while a bigger $K$ means the candidate lemma will more likely to be $\indf$, thus reducing the number of retries.
% \paragraph{Neural-symbolic intergration.}
% \tool training and inferencing are implemented in Python using PyTorch \cite{pytorch}. \spc, like most industry Model Checkers, is implemented in C++, creating a gap that needs to be addressed. We implement a client-server architecture in which \tool is wrapped in a gRPC server connects to a gRPC client inside \spc. This architecture hits a sweet spot between the ease of implementation and performance: it is possible, but much harder, to implement \tool using PyTorch's C++ binding. Alternatively, connecting \tool with \spc through a file-based architecture is easy, but creates large IO overhead.
